...
namespace Characters.Movement {
  public class SlidingMovement : AbstractMovement, IFallable, IJumpable {
    ...
    public override void Move(Vector3 direction) {
      ...
      // If there is no direction (aka input is 0) -> changing the state to
      // ground movement
      ...
      var velocity = CommonMethods.CreateVectorWithoutLoosingYWithMultiplier(
          direction, rbd.velocity.y, stats.slidingSpeed);
    ...
    private void UpdateAnimParameters(Vector3 groundVelocity) {
      ...
      animatorFacade.SetGroundVelocity(
          CommonMethods.CalculateGroundVelocity(groundVelocity));
      animatorFacade.SetSliding(true);
    }

    public override void CleanUp() { animatorFacade.SetSliding(false); }

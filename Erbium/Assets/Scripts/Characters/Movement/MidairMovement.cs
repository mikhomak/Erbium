...
namespace Characters.Movement {
  public class MidairMovement : AbstractMovement, IJumpable, IFallable {
    ...
    public override void SetUp() { _currentJumps = stats.maxJumps; }

    public override void Move(Vector3 direction) {
      // if the character is not in the air, change the movement state to ground
      // movement
      if (!IsFalling()) {
      ...
      AddVelocity(CommonMethods.CreateVectorWithoutLoosingYWithMultiplier(
          direction, rbd.velocity.y, stats.airSpeed));
    ...
    private void CalculateVelocity() {
      // If the character is falling too fast, clamping Y velocity to
      // stats.maxDownVelocity
      if (rbd.velocity.y < stats.maxDownVelocity) {
        rbd.velocity =
            CommonMethods.ModifyYinVector(rbd.velocity, stats.maxDownVelocity);
      } else // If not, then applying acceleration force down (gravity)
      {
        rbd.AddForce(Vector3.down * stats.additionalGravityForce,
    ...
    private void UpdateLandingAnimation(Vector3 direction) {
      // Caching the variable, so we only invoking setIsAboutToLand when the
      // value of oldAboutToLand has changed
      float yValue = rbd.velocity.y;
      if (yValue < 0 &&
          CommonMethods.IsAboutToLand(
    ...
    public override void CleanUp() { animatorFacade.UntoggleAirAnimations(); }

...
public static class CommonMethods {
  ...
  public static bool ONGround(Vector3 position) {
    ...
    return Physics.Raycast(origin, Vector3.down, out _, GroundRatDistance,
                           GroundLayer);
  }

  public static bool IsAboutToLand(Vector3 position, Vector3 direction,
                                   float normalizedValue) {
    var origin = getPositionRaycastLiftedForLanding(position);
    position.y += 0.2f;
    var length = getValueInRange(normalizedValue, LandingRatDistanceMIN,
                                 LandingRatDistanceMAX);
    Debug.DrawRay(origin, Vector3.down * length + direction * length,
                  Color.red);

    return Physics.Raycast(
        origin, Vector3.down * length + direction.normalized * length, out _,
        length, GroundLayer);
  }
  ...
  CreateVectorWithoutLoosingYWithMultiplier(Vector3 vector, float y,
                                            float multiplier) {

...
public class InputManager : MonoBehaviour {
  ...
  private void Start() { _player = playerGameObject.GetComponent<IPlayer>(); }
  ...
  private void Update() {
    ...
    if (Input.GetButtonDown($"Fire1")) {
      if (_player.getMovement() is GroundMovement || _player.getMovement()
                                                         is AttackingMovement) {
  ...
  public static float getHorInput() { return Input.GetAxis("Horizontal"); }

  public static float getVerInput() { return Input.GetAxis("Vertical"); }

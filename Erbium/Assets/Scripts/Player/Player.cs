...
public class Player : MonoBehaviour,
                      IPlayer,
                      IDamageDealer {
  private static readonly FastEnumIntEqualityComparer<MovementEnum>
  ...
  private void Start() {
    ...
    _animatorFacade =
        new AnimatorFacade(GetComponentInChildren<ICharacterAnimator>(), this);
    InitMovements();                             // creating all movements
    _movement = _movements[MovementEnum.Ground]; // setting the current movement
                                                 // as Ground One
  ...
  private IMovementDirection SetCameraDirection(CameraView cameraView) {
    ...
    return cameraView switch {CameraView.AlwaysForward =>
                                  new ThirdPersonCameraDirection(),
                              _ => new ThirdPersonCameraDirection()};
  }

  public IHealthComponent getHealthComponent() { return _healthComponent; }

  public IAnimatorFacade getAnimatorFacade() { return _animatorFacade; }

  public IArmour getArmour() { return _armour; }

  public IAttackManager getAttackManager() { return _attackManager; }

  public Rigidbody getRigidbody() { return _rbd; }

  public Transform getTransform() { return transform; }

  public IMovement getMovement() { return _movement; }
  ...
  public void ChangeMovement(MovementEnum movementEnum) {
    ...
    _movement = _movements[movementEnum]; // changing the current movement to
                                          // the new one
  ...
  /// That way we don't need to create a new movement each time we change the
  /// movement state We can just use movements[MovementEnum] to get each
  /// movement We are using this array in changeMovement(MovementEnum)
  /// </summary>

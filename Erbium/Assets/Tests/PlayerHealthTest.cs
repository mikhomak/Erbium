...
public class PlayerHealthTest : AbstractTest {
  ...
  public new void SetUpTestScene() {
    _projectileGo = Object.Instantiate(
        Resources.Load<GameObject>("Prefabs/Enemies/Projectile"));
  ...
  public IEnumerator PlayerTakeDamage() {
    ...
    Assert.True(Math.Abs(healthBeforeTakingDamage - player.getStats().health) >
                0.2f);
  }
  ...
  public IEnumerator PlayerTakeDamageWithPhysicArmour() {
    ...
    Assert.True(Math.Abs(healthBeforeTakingDamage - damageInfo.damage +
                         player.getStats().physicArmour -
